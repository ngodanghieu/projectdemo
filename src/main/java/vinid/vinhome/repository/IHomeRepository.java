...
public interface IHomeRepository extends JpaRepository<Home,Long> {
    ...
    @Query(nativeQuery = true,
            value = "SELECT home.* FROM home INNER JOIN adress_home ON home.home_id = adress_home.home_id INNER JOIN building ON adress_home.building_id = building.building_id INNER JOIN adress ON adress.adress_id = building.adress_id INNER JOIN district ON district.district_id = adress.district_id INNER JOIN city ON city.city_id = district.city_id WHERE city.code = :codecity OR adress.content_detail = :adress OR district.code = :codedistrict")
